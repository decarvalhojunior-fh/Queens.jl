...
module queens_serial{
	...
	proc queens_serial_regular(const size: uint(16)): (uint(64),uint(64)){

		var control: [0..MAX] bool = false;
		...
		while(true){
			...
			if board[depth] == size then
				...
				if (stillLegal(board, depth) && !control[board[depth]]) {

					control[board[depth]] = true;
					depth +=1;
					tree_size+=1;
